...
$(document).on('click.inline-edit', 'a.inline-edit', function(e) {
  ...
  $(document).on('click', '[id^="cat-id-"]', function () {
    if (!localStorage.getItem('reloadOnce')) {
      var catid = $(this).data('id');  
      var name = $(this).data('name');  
        
        localStorage.setItem('cat-id', catid);
        localStorage.setItem('reloadOnce', 'true'); // Set flag
        window.location.href = 'kb1.php?catid=' + catid+ '&name=' + name;
    }
});
...
function loadHelpTopic(topicId, element) {
  document.getElementById('topicId').value = topicId;

  // Remove selection from all
  document.querySelectorAll('.topic-tree li').forEach(li => li.classList.remove('selected'));
  ...
  $.ajax('ajax.php/form/help-topic/' + topicId, {
      ...
      success: function(json) {
        ...
        $('#dynamic-form1').each(function() {
          const $form = $(this);

          // Empty the tbody and append the new HTML and media
          $form.empty().append(json.html);

          // Add media if any
          $(document.head).append(json.media);

          // Select all rows within the form
          ...
          for (let i = 0; i < $rows.length; i++) {
              ...
              // Check if the row contains a header <th>
              if ($row.find('th').length > 0) {
                  // If it's a header row, span the <th> across 12 columns
                  const $header = $('<tr></tr>').append($row.find('th').attr('colspan', 12));
                  $newRows.push($header);
                  continue; // Skip this row and move to the next
              }

              const $tds1 = $row.find('td');

              // If the row has only one <td>, just add it as is, with colspan of 3
              if ($tds1.length === 1) {
                  $newRows.push($('<tr></tr>').append($tds1.attr('colspan', 6)));
                  continue;
              }

              // For each <td>, apply colspan of 3
              $tds1.each(function() {
                  $(this).attr('colspan', 3);  // Set colspan to 3 for each <td>
                  
                  // Apply the custom CSS if the <td> contains a <select> box
              ...
              // Try to pair with the next row only if it has more than one <td>
              const $tds2 = $rows.eq(i + 1)?.find('td');
              if ($tds2 && $tds2.length > 1) {
                  // Ensure the second row also has the correct colspan
                  $tds2.each(function() {
                      $(this).attr('colspan', 3);  // Apply colspan to the second row's td as well
                  });

                  const $newRow = $('<tr></tr>').append($tds1).append($tds2);
                  $newRows.push($newRow);
                  i++; // Skip the next row since it's already used
              } else {
                  // If can't pair, just add the current row as standalone
                  $newRows.push($('<tr></tr>').append($tds1));
              }
          }

          // Replace with the rearranged rows inside the tbody with id "dynamic-form1"
          $form.empty().append($newRows);
        });
...
//   // Toggle behavior for the parent item and its children when clicking the toggle icon
//   document.querySelectorAll('.topic-tree .toggle').forEach(toggle => {
//       toggle.addEventListener('click', function (e) {
//           e.stopPropagation(); // Prevent triggering other clicks (like on the li itself)
//           toggleChildrenVisibility(this);
//       });
//   });

//   // Add event listener for clicking the text (link or topic) to toggle the visibility of children
//   document.querySelectorAll('.topic-tree .topic-item').forEach(item => {
//       item.addEventListener('click', function (e) {
//           // If the click wasn't on the toggle, trigger the toggle for this topic
//           if (!e.target.classList.contains('toggle')) {
//               // Don't propagate the click to the parent item
//               e.stopPropagation();
//               toggleChildrenVisibility(this.querySelector('.toggle'));
//           }
//       });
//   });

//   // Function to toggle children visibility
//   function toggleChildrenVisibility(toggle) {
//       const children = toggle.closest('li').querySelector('.children');
//       if (children) {
...
document.addEventListener('DOMContentLoaded', function () {
  // Attach a single event listener to the parent container (e.g., body or a specific static container)
  document.body.addEventListener('click', function (e) {
    // Check if the clicked element is a toggle
    if (e.target.classList.contains('toggle')) {
      e.stopPropagation(); // Prevent triggering other clicks (like on the li itself)
      toggleChildrenVisibility(e.target);
    }

    // If the clicked element is a topic-item but not a toggle, trigger the toggle for this topic
    const topicItem = e.target.closest('.topic-item');
    if (topicItem && !e.target.classList.contains('toggle')) {
      e.stopPropagation();
      const toggle = topicItem.querySelector('.toggle');
      if (toggle) toggleChildrenVisibility(toggle);
    }
  });
  ...
  $('.dynamic-forms').each(function () {
    ...
    for (let i = 0; i < $rows.length; i++) {
        ...
        $tds1Cloned.each(function () {
            ...
            // Add hide class to select2-selection select2-selection--single
            $(this).find('.select2-selection.select2-selection--single').addClass('hide');
            
            // Remove the select2-hidden-accessible class from each select box
