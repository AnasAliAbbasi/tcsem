...
if($_REQUEST['q'] || $_REQUEST['cid'] || $_REQUEST['topicId']) { //Search.
    ...
    if (count($categories)) {
        ...
        echo '<div>'.__('Click on the category to browse FAQs or manage its existing FAQs.').'</div><div class="clear"> </div>
        <ul class="tabs clean faqs" id="faqs_tabs">';
        foreach ($categories as $index => $C) {

            if($C->getNumFAQs() > 0) {
                echo sprintf(
                    '<li>
                        <a class="truncate" id="cat-id-%1$s" data-name="%2$s"  style="max-width:600px" data-id="%1$s" href="#FaqArea">%2$s - (%3$s)</a>
                    </li>',
                    $index,
                    htmlspecialchars($C->getLocalName()),
                    $C->getNumFAQs()
                );
            }
        ...
        echo '<div id="faqs_tabs_container">
            <div id="FaqArea" class="tab_content" >
                <h4 style="margin-left: 24px;
                    margin-top: 9px;
                    font-weight: bold;
                    "> '; echo $name;
                    echo ' </h4> <hr />'; 
                    ...
                    if($categories[$catid]->children->exists(true)){
                        foreach ($categories[$catid]->children as $c) {
                        
                            echo sprintf('<div style="margin-left: 38px;margin-top: 15px;">
                            
                           <h3> <a 
                                    style="
                                    text-decoration: none;
                                    font-weight: bold;" href="#%d">%s (%d)</a>  - <span>%s</span> </h3></div>',
                                    $c->getId(),
                                    $c->getLocalName(),
                                    $c->getNumFAQs(),
                                    ...
                                    foreach ($c->faqs as $faq) {
                                        ...
                                        if ($show)
                                        echo sprintf('
                                            <div style="margin-left: 100px;margin-top: 15px;"> <a style="font-size: 16px;
                                    text-decoration: none;
                                    font-weight: bold;" href="faq.php?id=%d" class="previewfaq">%s <span>- %s</span></a> %s</div>',
                        ...
                        foreach($categories[$catid]->faqs as $faq){
