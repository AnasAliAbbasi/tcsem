...
{  
    $today = date('Y-m-d');
    $today = date('Y-m-d', strtotime($today . ' - 60 days'));
	...
	AND _wo.WoNumber>17153
    AND _wo.RMAFlagCode != 1
    AND _wo.WONumber IN
    ( select wo_number from _wo_cron_logs where WO_Number in ( 	select wonumber from manex_work_order_documents where DATE(UpdatedUTC) >= "%2$s") group by wo_number having count(*) <= 3  ) 
