...
$woticketconditionStencil = array(
...
processWOTickets($settings, $woticketconditionStencil , $woticketconditionSMT);

function processWOTickets($settings, $woticketconditionStencil , $woticketconditionSMT)
{
    try{
        ...
        if (isValidArray($customdata)) {
            ...
            foreach ( $customdata as $index => $category){
                foreach ($category as $cs) {
                    $revision = 'No';

                    if ($index == 'stencil'){
                        $topicIds = $woticketconditionStencil[$cs['_wo_saletype']][$cs['_repeat_flag']][$revision];
                    }else{
                        $topicIds = $woticketconditionSMT[$cs['_wo_saletype']][$cs['_repeat_flag']][$revision];
                    }
...
{  
    $today = date('Y-m-d');
    $today = date('Y-m-d', strtotime($today . ' - 21 days'));
    ...
    and _wl.topic_id = 21 
    and _sfe.form_id = 17
    and _sfev.field_id = 149
    and _sfev.value like "%%Yes%%"
    and DATE(_st.created) >= "%2$s";', $fields , $today);


    $result = executeQuery($query);
    $stencil =  getDataFromResultSet($result);
    ...
    and _wl.topic_id = 21 
    and _sfe.form_id = 17
    and _sfev.field_id = 150
    and _sfev.value like "%%Yes%%"
    and DATE(_st.created) >= "%2$s";', $fields , $today);

    $result = executeQuery($query);
    $smt =  getDataFromResultSet($result);
...
// select * from sem_ticket a 
// INNER join sem_help_topic_form b on a.topic_id = b.topic_id
// inner join sem_form_entry c on b.form_id = c.form_id
// inner JOIN sem_form_entry_values d on c.id = d.entry_id
// where  a.ticket_id = '4943' 
// and b.form_id = 2
// and d.field_id = 46
// and a.topic_id = 16
// and d.value like '%03-034720-00%';
