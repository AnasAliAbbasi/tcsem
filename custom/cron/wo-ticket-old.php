...
$processedCombinations = ['18324-20']; // Array to track processed (won, topicId) pairs

foreach ($topicIds as $topicId) {
    if ($topicId == 25 && $cs['_rma_flag'] == 1) {
        echo "RMA flag is 1 (active) Not Create BOM " . $cs['won'];
    } else {
        // Create a unique combination of cs['won'] and topicId
        $currentCombination = $cs['won'] . '-' . $topicId;

        // Skip if the combination has been processed before
        if (!in_array($currentCombination, $processedCombinations)) {

            // Mark the combination as processed
            $processedCombinations[] = $currentCombination;

   

            // Fetch document and handle ticket creation
            $document = getDocumentAvailable($cs['won']);
            $cs['_utc_time'] = (!empty($document) ? $document[0]['Document_Folder'] : 'Document Not Available Now');
            $topicTitle = $settings['topicId'][$topicId];

            sleep(1);

            HitLogsToFile("creating ticket", $topicId . '-' . $topicTitle, $cs['won']);
            ...
            HitLogsToFile("skipped duplicate ticket creation", $topicId . '-' . $topicTitle, $cs['won']);
        }
    }
}
...
function getDataFromDB()
{
    ...
    $today = date('Y-m-d');
	$today = date('Y-m-d', strtotime($today . ' - 40 days'));
...
function generateWOLog($ticketId, $topicId, $data)
{
    ...
    if (!$result) {
        logErrorToFile("work order log generate error" , $topicId , $data['won'] , $ticketId);
    }
...
function HitLogsToFile($message, $topicId = null, $woNumber = null) {
